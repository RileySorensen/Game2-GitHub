...
namespace Bogo
{
	...
	inline Vector2 Matrix3x3::operator*(const Vector2& v) const
	{
		...
		result.x = v.x * rows[0][0] + v.y * rows[0][1] + 1.0f * rows[0][2] + 1.0f * rows[0][2];
		result.y = v.x * rows[1][0] + v.y * rows[1][1] + 1.0f * rows[1][2] + 1.0f * rows[1][2];
	...
	inline Matrix3x3 Matrix3x3::operator*(const Matrix3x3& mx) const
	{
		...
		//row3
		result[2][0] = rows[2][0] * mx[2][0] + rows[2][1] * mx[1][0] + rows[2][2] * mx[2][0];
		result[2][1] = rows[2][0] * mx[2][1] + rows[2][1] * mx[1][1] + rows[2][2] * mx[2][1];
		result[2][2] = rows[2][0] * mx[2][2] + rows[2][1] * mx[1][2] + rows[2][2] * mx[2][2];
